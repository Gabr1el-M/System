...
export default function Posts({ posts: postsBlog, page, totalPage }: PostsProps) {
   ...
   async function navigatePage(pageNumber: number) {
    ...
    return <>
        <Head>
            <title>Blog | Gabriel Martins</title>
        </Head>
                ...
                <Link key={post.slug} href={`/posts/${post.slug}`} legacyBehavior>
                    <a key={post.slug}>
                        <Image
                            ...
                            height={410}
                            placeholder='blur'
                            blurDataURL="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mNU01RbBQACEwEhAaCyFgAAAABJRU5ErkJggg=="
                            quality={100}
                        />
                ...
                <div className={styles.buttonNavigate}>
                    { Number(currentpage) >= 2 && (
                        ...
                        <button onClick={() => navigatePage(1)}>
                            <FiChevronsLeft size={25} color="white" />
                        </button>
                   ...
                   {Number(currentpage) < Number(totalPage) && (
                     ...
                     <button onClick={() => navigatePage(Number(totalPage))}>
                         <FiChevronsRight size={25} color="white" />
                     </button>
                 </div>
                   )}
                </div>
